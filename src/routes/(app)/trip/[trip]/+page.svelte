<script lang="ts">
	...
	function checkIfHistory(timestamp: string) {
...
<TripView mode="trip" tripId={data.trip.id}>
	...
	<main class="grid w-full content-start gap-12 justify-self-center">
				...
				{#each data.stay as stay}
					<div
						class="{checkIfHistory(stay.checkIn)
							? 'opacity-[0.6]'
							: undefined} bg-base-200/70 border-base-300/30 grid rounded-xl border shadow-md"
					>
						<div class="bg-base-400 text-primary-content rounded-t-xl p-2 text-center">
		...
		<section class="grid w-full grid-cols-2 gap-4 lg:grid-cols-2 xl:grid-cols-3">
			<h2
				class="font-header col-span-2 grid grid-cols-[1fr_auto] text-4xl font-bold lg:col-span-2 xl:col-span-3"
			>
				<a href="/trip/{data.trip.id}/stay">Pack</a>
			</h2>

			<div class="bg-base-200/70 border-base-300/30 grid content-start rounded-xl border shadow-md">
				<div
